...
#include "Car.h"
#include "IR.h"
#define D1      !GPIO_getInputPinValue(GPIO_PORT_P3,GPIO_PIN4)
#define D2      !GPIO_getInputPinValue(GPIO_PORT_P3,GPIO_PIN3)
...
int8_t j1=2,j2=2,j3=3,j4=3,j5=4,j6=5,j7=5,j8=5;
void Back(uint8_t backflag)//backflag是中断返回的房间数（Number）
{
    ...
    if(backflag == 6)               //表示是6号房间
    {
        ...
        {
            j6--;//遇见一次黑线就自减一  第一次遇见黑线 j6=3（左转）     第二次遇见黑线 j6=2（左转）     第三次遇见黑线j6=1 (直走)     第四次遇见黑线 j6=0 (直走)  第五次遇见黑线 j6<0 (停止)
            if(j6==3|j6==2)
            {
                left();delay_ms(300);//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                while(!((!D1) & D2 & D3 &(!D4)) ){left();}
